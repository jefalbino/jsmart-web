...
function Keypad() {
	...
	this.addKeyDef('HALF_SPACE', 'half_space');
	...
	this.qwertyAlphabetic = ['qwertyuiop', 'asdfghjklç', 'zxcvbnm'];
	// Standard US keyboard layout
	this.fullLayout = ['!@#$%^&*()_=' + this.HALF_SPACE + this.SPACE + this.CLOSE,
		this.HALF_SPACE + '`~[]{}<>\\|/' + this.SPACE + '789',
		'qwertyuiop\'"' + this.HALF_SPACE + '456',
		'asdfghjklç;:' + this.HALF_SPACE + '123',
	...
	this.qwertyLayout = [this.qwertyAlphabetic[0] + this.CLOSE,
		this.qwertyAlphabetic[1] + this.CLEAR,
		this.SPACE + this.qwertyAlphabetic[2] + this.SHIFT + this.BACK];
	this.numericLayout = ['123' + this.CLOSE, '456' + this.CLEAR, '789' + this.BACK, this.SPACE + '0'];
	this.regional = []; // Available regional settings, indexed by language code
	this.regional[''] = { // Default regional settings
		buttonText: '...', // Display text for trigger button
		closeText: 'Close', // Display text for close link
		closeStatus: 'Close the keyboard', // Status text for close link
		...
		fullLayout: this.fullLayout, // Default layout for full keyboard
	...
	this._defaults = { // Global defaults for all the keypad instances
		...
		layout: this.numericLayout, // Layout of keys
		separator: '', // Separator character between keys
		target: null, // Input target for an inline keypad
		keypadOnly: false, // True for entry only via the keypad, false for real keyboard too
...
$.extend(Keypad.prototype, {
	...
	_attachPlugin: function(target, options) {
		...
		if (inline) {
			...
			inst._mainDiv.css({'display':'inline-block'});
		}
	...
	_connectKeypad: function(target, inst) {
		...
		if (!inst._inline) {
			...
			if (inst.options.showOn == 'button' || inst.options.showOn == 'both') {
				...
				$('button[target="' + target.attr('id') + '"]').each(function() {
					$(this).remove();
				});
